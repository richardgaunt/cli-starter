...
// Import our functions directly
import { getRecentCommits, checkGitRepository, checkGhCli, createPR } from '../index.js';
...
describe('Git utility functions', () => {
  ...
  afterEach(() => {
    ...
    global.input = originalInput;
    global.confirm = originalConfirm;
    global.twig = originalTwig;
  });
  ...
  describe('createPR', () => {
    it('should create a PR successfully without target branch', async () => {
    ...
    it('should create a PR successfully with target branch', async () => {
      execSync.mockReturnValueOnce(Buffer.from('https://github.com/user/repo/pull/123'));

      const result = await createPR('Test PR', 'PR body', 'develop');

      expect(execSync).toHaveBeenCalledWith('gh pr create --title "Test PR" --body "PR body" --base "develop"');
      expect(result).toEqual({
        success: true,
        url: 'https://github.com/user/repo/pull/123'
      });
